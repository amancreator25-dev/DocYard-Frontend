...
const DocumentDetails = () => {
  ...
  useEffect(() => {
    ...
    const loadInteractions = async () => {
      try {
        ...
        const countResponse = await getLikeCount(
          document._id
        );
        ...
        setLikeCount(
          countData?.data?.likeCount ??
            countData?.likeCount ??
            0
        );

        // --------------------------------------
        // PERSONAL INTERACTION STATUS
        ...
        if (bookmarkResponse.status === "fulfilled") {
          const data = bookmarkResponse.value;
  ...
  useEffect(() => {
    ...
    const loadComments = async () => {
      ...
      try {
        const response = await getDocumentComments(
          document._id
        );
  ...
  const handleBookmark = async () => {
    if (!isAuthenticated || !currentUser) {
      setError(
        "Please sign in to save documents."
      );
      return;
    }
  ...
  const handleLike = async () => {
    if (!isAuthenticated || !currentUser) {
      setError(
        "Please sign in to like documents."
      );
      return;
    }
    ...
    <main className="min-h-screen bg-paper text-ink">
      ...
      <section className="w-full px-6 pb-12 pt-12 sm:px-10 md:px-14 md:pb-14 md:pt-16 lg:px-20 xl:px-24">
        <div className="w-full">
          ...
          <Link
            to="/documents"
            className="inline-flex !h-12 items-center justify-center rounded-lg border border-[#0A3A63] bg-[#0A3A63] px-7 text-sm font-semibold !text-[#ffffff] shadow-sm transition-all hover:border-ink hover:bg-ink"
          >
            Back to archive
          </Link>
      ...
      <section className="w-full px-6 pb-20 sm:px-10 md:px-14 md:pb-24 lg:px-20 xl:px-24">
        <div className="w-full">
          ...
          {isAuthenticated && currentUser ? (
            ...
            <div className="flex w-full items-center justify-between gap-5 rounded-xl border border-line bg-paper-raised px-6 py-5">
              ...
              <div>
                ...
                <p className="mt-1 text-sm text-ink-soft">
                  Sign in to like, save, and comment on documents.
                </p>
              ...
              <Link
                to="/login"
                className="shrink-0 rounded-md bg-[#0A3A63] px-5 py-2.5 font-mono text-[10px] font-semibold uppercase tracking-wider !text-[#ffffff] transition hover:bg-[#082F50]"
              >
                Sign in
              </Link>
                      ...
                      editingComment === item._id ? (
                        <div className="mt-5">
                          ...
                          <textarea
                            ...
                            rows={3}
                            className="w-full resize-y rounded-md border border-line bg-paper px-4 py-3 text-base leading-7 text-ink outline-none transition focus:border-[#0A3A63] focus:ring-2 focus:ring-[#0A3A63]/10"
                          />
                          ...
                          {isCommentOwner && (
                            <div className="mt-5 flex flex-wrap gap-2">
                              ...
                              <button
                                ...
                                className="inline-flex h-9 items-center rounded-md border border-line bg-[#0A3A63] px-4 text-sm font-medium text-white transition hover:bg-[#082F50]"
                              >
                                Edit
                              </button>
                              ...
                              <button
                                ...
                                className="inline-flex h-9 items-center rounded-md border border-line bg-[#0A3A63] px-4 text-sm font-medium text-white transition hover:border-red-300 hover:bg-red-600"
                              >
                                Delete
                              </button>
